...
export function useAuthFlow() {
  ...
  const completeAuthentication = (response: AuthResponse, persistent = true) => {
    tokenStorage.save(response, persistent)
    window.location.assign(getPostAuthRoute(response))
  }
  ...
  const login = async (payload: LoginRequest, keepSignedIn: boolean) => {
    ...
    try {
      ...
      completeAuthentication(response, keepSignedIn)
    } catch (requestError) {
  ...
  const verifyEmail = async (payload: VerifyEmailRequest) => {
    ...
    try {
      ...
      setError(getApiErrorMessage(requestError, 'That code is invalid or expired.'))
    } finally {
      setPending(false)
    }
  }
  ...
  const resendCode = async () => {
    ...
    try {
      ...
      setError(getApiErrorMessage(requestError, 'We could not resend the code.'))
    } finally {
      setPending(false)
    }
  }
